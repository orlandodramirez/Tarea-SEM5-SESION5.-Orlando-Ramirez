...
class matriculaModel {
    ...
    public function insertar($fecha,$idEstudiante,$idUsuario,$idCurso) {
        $sql = 'INSERT INTO matriculas VALUES (0,:fecha,:idEstudiante,:idUsuario,:idCurso)';
        $statement = $this->PDO->prepare($sql);
    ...
    public function actualizar($idMatricula,$fecha,$idEstudiante,$idUsuario,$idCurso) {
        $sql = 'UPDATE matriculas SET fecha=:fecha,idEstudiante=:idEstudiante,idUsuario=:idUsuario,idCurso=:idCurso WHERE idMatricula=:idMatricula';
        $statement = $this->PDO->prepare($sql);
        $statement->bindParam(':fecha',$fecha);
        $statement->bindParam(':idEstudiante',$idEstudiante);
        $statement->bindParam(':idUsuario',$idUsuario);
    ...
    public function cargarDesplegableEstudiantes() {
        $sql = 'SELECT idEstudiante,CONCAT(nombre," ",apellido) AS estudiante FROM Estudiantes ORDER BY idEstudiante ASC';
        $statement = $this->PDO->prepare($sql);
        return ($statement->execute()) ? $statement->fetchAll() : false;
    }
